...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("OrderBook Rust - Simple Performance Test");
    println!("==========================================\n");
    ...
    // test 4: Best bid/ask performance
    println!("\nTest 4: Best Bid/Ask Performance");
    println!("----------------------------------");
    
    let start = Instant::now();
    for _ in 0..10_000 {
        let _best_bid = order_book.get_best_bid();
        let _best_ask = order_book.get_best_ask();
    }
    let duration = start.elapsed();
    
    println!("Got 20,000 best bid/ask queries in {:.2}ms", duration.as_millis());
    println!("   Rate: {:.0} queries/second", 20_000.0 / duration.as_secs_f64());
